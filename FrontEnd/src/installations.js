...
export default class Practicas extends React.Component {
  state = {
    persons: []
  }

  componentDidMount() {
    axios.get(`http://localhost:4000/api/installations`)
      .then(res => {
      ...
      <ul>
        { this.state.persons.map(person => 
                  ...
                  {/* <img src={person.images[0] || defaultImg} alt="single room" className="img-fluid"/> */}
                      ...
                      <Link to={`/rooms/${person.description}`} className="btn-warning room-link text-center" >Features</Link>
